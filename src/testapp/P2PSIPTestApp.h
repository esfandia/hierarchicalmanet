...
public:
    ...
    static cNEDValue sipid(cComponent *context, cNEDValue argv[], int argc) {
        OverlayKey key = OverlayKey::random();
        std::string id{"sip:"};
        id.append(argv[0].stringValue());
        id.append("@");
        id.append((argc == 2) ? argv[1].stringValue() : "example.com");
        return id;
    }
};

Define_NED_Function(P2PSIPTestApp::randomIdentifier,"string randomIdentifier(int length?)")
Define_NED_Function(P2PSIPTestApp::sipid,"string sipid(string name, string domain?)")
